...
|--------|---------|---------|
| `GMOMaintenance` | 1.8/1.9 | SCCM maintenance, package deployments |
| `GMOMaintV2` | 2.0 | Enhanced computer info + maintenance |
| `GMOComputerInfo` | 2.3.3/2.4.3 | Computer inventory + update management |

## GMOComputerInfo — Get-GMOComputerInfo Parameters
```powershell
Get-GMOComputerInfo `
    ...
    -OutPath "C:\Temp\GMOComputerInfo"

# CollectionID pattern: ^[A-Z]{3}\w{5}$  (e.g., GMO00001)
```

## GMOComputerInfo — Information Collected
```powershell
...
## GMOMaintenance — Install-GmoCMPackageOnDemand
```powershell
Install-GmoCMPackageOnDemand `
    -PackageID "GMO00001" `              # Mandatory
    -CollectionID "GMO00002" `           # Mandatory
...
$Script:CMSiteServer = "bosinfprdmsc101.gmo.tld"
$Script:CMSiteName   = "GMO"
```
...
@{
    ...
    RootModule        = 'GMOMaintenance.psm1'
    FunctionsToExport = '*'
...
# Placeholder: "1.8.0" in GMOMaintenance, "2.3.3" in GMOComputerInfo
```
...
.\Manage-AZVMSnapshot -SubscriptionName GMOPrimary `
    -VMName Apawar-EU2 `
...
|-------|----------|--------|
| `KillDeployment` | Remove-SCCMDeployment | GMOMaintenance |
| `gwmisize` | Get-WMIObjectSize | GMOMaintenance |
| `getdlsize` | Get-DownloadSize | GMOMaintenance |
| `gmocomps` | Get-GMOComputerInfo | GMOComputerInfo |

## Exported Functions per Module
**GMOComputerInfo:** Get-GMOComputerInfo, Get-GMOMachineKBInfo, Get-GMOUserLogonStatus, Get-GMOPostProvisionValidation, Get-GMOCloudDesktopDeploymentInfo, Get-GMOWin7CompsFromEmail
