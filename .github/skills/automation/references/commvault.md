...
## Server & Endpoints
```
Base URL:  https://commvault-server/commandcenter/api
...
ASTRAL_TABLES = [
    ...
    'gdm-index-market-data', 'gdm-issuer-master', 'gdm-security-master', 'corp-security-master-feed',
...
$key  = Get-Content "\\fileserver\SysConfig\Storage\svc-backup-automation\svc-backup-automation.key"
$pass = Get-Content "\\fileserver\SysConfig\Storage\svc-backup-automation\svc-backup-automation.txt" |
        ConvertTo-SecureString -Key $key
$cred = New-Object PSCredential("svc-backup-automation", $pass)
Connect-CVServer -Server commvault-server -Credential $cred

# Client filtering
$clients = Get-CVClient | Where-Object {
    $_.name -notmatch '_' -and
    ($_.name -match 'SMB|NFS|BKP|CORP') -and
    $_.name -notmatch 'FAS'
}
...
- Deploy to: `/mnt/automation/nfs/backupautomation/commvault/` on marprdnfs001
