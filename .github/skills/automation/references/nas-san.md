...
# -SharePath  : target share (default: \\fileserver\UserHome)
```
...
Connect-VIServer -Server "usvc.corp.example.com"
...
for CLUSTER in "${CLUSTERS[@]}"; do
    ssh admin@$CLUSTER "aggr show -fields aggregate,node,size,used,available,percent-used" \
        >> /mnt/automation/nfs/storageautomation/outputs/aggr_capacity_$(date +%Y%m%d).csv
done

# netapp_volume_capacity_report.sh
for CLUSTER in "${CLUSTERS[@]}"; do
    ssh admin@$CLUSTER "volume show -fields vserver,volume,size,used,available,percent-used" \
        >> /mnt/automation/nfs/storageautomation/outputs/vol_capacity_$(date +%Y%m%d).csv
done
```
...
.\dfsn-dfsr -all `
    ...
    -rootname "\\corp.example.com\Path" `
