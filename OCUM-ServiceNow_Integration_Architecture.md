...
- **Location**: `\\gmo\dsl\SysConfig\ServiceNow\SNOW.psm1`
- **Functions**:
...
- **Network Share Access**: Required for accessing SNOW module at `\\gmo\dsl\SysConfig\ServiceNow\SNOW.psm1`
...
- **Primary Account**: `GMO\service-netapp`
...
```powershell
# Services configured to run as GMO\service-netapp:
...
1. **Service Account Setup**:
   - Created `GMO\service-netapp` domain service account
   ...
   - Changed service "Log On" properties from Local System to `GMO\service-netapp`
...
2. **Permission Validation**:
   ...
   - **Tested access to network SNOW module path**: `\\gmo\dsl\SysConfig\ServiceNow\SNOW.psm1`
...
- **SNOW Module Path**: `\\gmo\dsl\SysConfig\ServiceNow\SNOW.psm1`
...
|-------------|-----|---------|--------|
| gmodev | gmodev.service-now.com | Development/Testing | Active |
| gmotest | gmotest.service-now.com | User Acceptance Testing | Active |
| gmo | gmo.service-now.com | Production | Ready |
...
- **Service Account**: `GMO\service-netapp` with minimal required permissions
...
1. **Create Service Account**:
   - Create `GMO\service-netapp` domain service account
...
2. **Configure OCUM Services**:
   ...
   - Modify service "Log On" properties to use `GMO\service-netapp`
...
|-------|----------|------------|
| Service Account Configuration | OCUM services fail to start | Verify "Log on as a service" rights for GMO\service-netapp |
| SNOW Module Access | Import failures, access denied | Verify GMO\service-netapp is member of DSL_SysConfig_ServiceNow_Read group |
| Network Path Access | Cannot access \\gmo\dsl\SysConfig path | Check DSL_SysConfig_ServiceNow_Read group membership and network connectivity |
...
whoami
# Should return: GMO\service-netapp when run from OCUM service context
...
Test-NetConnection -ComputerName "gmodev.service-now.com" -Port 443

# Test incident creation
.\SNWNewIncidentModule.ps1 -correlationid "TEST123" -ServiceNowEnvironment gmodev

# Test incident closure
.\SNWCloseIncidentModule.ps1 -CorrelationId "TEST123" -ServiceNowEnvironment gmodev
...
Test-Path "\\gmo\dsl\SysConfig\ServiceNow\SNOW.psm1"
...
SNOW Module: \\gmo\dsl\SysConfig\ServiceNow\SNOW.psm1
