...
|  ONTAP Clusters (x6)      |<------------------------------|                 |
|  cluster-06             |                               |                 |
|  cluster-04             |                               |  Azure DevOps   |
|  cluster-05             |       Azure REST API          |  SharedBuild    |
|  cluster-02             |<------------------------------|  Windows Agent  |
|  cluster-03             |                               |  (svc-prdtfsbld)|
|  cluster-01             |                               |                 |
+---------------------------+                               |                 |
                                                            |  volume_        |
+---------------------------+       Azure REST API          |  reporting_     |
|  Azure NetApp Files (ANF) |<------------------------------|  main.py        |
|  anf-dr-account (centralus)  |                               |                 |
|  anf-primary-account (eastus2)    |  Azure Monitor Metrics API    |                 |
+---------------------------+<------------------------------|                 |
                                                            |                 |
+---------------------------+  UNC file read (CSV reports)  |                 |
|  Commvault Reports        |<------------------------------|                 |
|  (\fileserver\automation\...\reports)  |                               +--------+--------+
+---------------------------+                                        |
|  commvault-server             |          Graph API (HTTPS)             |
|  commvault-server             |                            +-----------v---------+
+---------------------------+                            |  SharePoint Online  |
                                                         |  Platform Eng site  |
                                                         |                     |
...
```python
ANF_ACCOUNTS = [
    { "account_name": "anf-dr-account", "resource_group": "centralus-anf-dr-account-rg",  "region": "centralus", "label": "CUS" },
    { "account_name": "anf-primary-account", "resource_group": "eastus2-anf-primary-account-rg",    "region": "eastus2",   "label": "EU2" },
]
```
...
|----------|------|-------------|
| P1 | Exact path match | Extract the last segment from each UNC content path. e.g. `UNC-NT_anf-source.corp.example.com\eu2-x-ats-betas` → volume `eu2-x-ats-betas`. Case-insensitive. First match wins. |
...
**`backup_configured` values:** `True` = matched + recent backup date; `False` = no match; `NA` = DP/LS volume, system/infra volume, anf-primary-account ANF, or matched but no recent date (see Section 8 for full detail).

**`CV_REPORT_DIR` (from `config.py`):**
```python
CV_REPORT_DIR = os.environ.get("CV_REPORT_DIR", r"\fileserver\automation\backupautomation\commvault\reports")
```
...
**Authentication:** Client credentials flow against Graph (`https://graph.microsoft.com/.default`), using `Corp Storage Volume Reporting` app registration with `Sites.ReadWrite.All` (admin consent granted).
...
| 2 | `CopyFiles@2` | Copy `Storage-Reporting/**` to build share `\\fileserver\Build\DevOps\...` |
...
| 4b | `DownloadSecureFile@1` | Download `svc-netapp_id_rsa` from Secure Files; exposes path as `$(sshKey.secureFilePath)` |
...
|----------|--------|---------|
| `NETAPP_SSH_USER` | Pipeline variable (`svc-netapp`) | ONTAP SSH |
...
| `CV_REPORT_DIR` | Pipeline variable (`\fileserver\automation\backupautomation\commvault\reports`) | Path to Commvault daily CSV exports |
...
| Property | Value |
|----------|-------|
| Display name | Corp Storage Volume Reporting |
...
| Subscription | `<your-subscription-id>` (corp-primary) |
...
| Hostname | Location |
|----------|----------|
| `cluster-06` | Ashburn (US-East) |
| `cluster-04` | EU2 |
| `cluster-05` | EU2 |
| `cluster-02` | Marlborough backup |
| `cluster-03` | Marlborough NAS |
| `cluster-01` | Sydney |

- **SSH user:** `svc-netapp`
- **Auth:** Private key at `/opt/svc-netapp/.ssh/id_rsa` on Linux; uploaded to Azure DevOps Secure Files as `svc-netapp_id_rsa` for Windows agent use.

### ANF Accounts

| Account | Resource Group | Region |
|---------|---------------|--------|
| `anf-dr-account` | `centralus-anf-dr-account-rg` | Central US |
| `anf-primary-account` | `eastus2-anf-primary-account-rg` | East US 2 |

### Commvault

| Property | Value |
|----------|-------|
| Server | `commvault-server` (Commvault v11.40.26) |
| Report directory (UNC) | `\fileserver\automation\backupautomation\commvault\reports` |
...
| Property | Value |
|----------|-------|
| Site URL | `https://yourorg.sharepoint.com/sites/teams_platformengineering` |
| Library | Documents |
...
| `NA` | Not applicable — either: (a) volume is a DP/LS type (SnapMirror replica — never directly backed up), (b) volume is a system/test/infra volume not expected to be backed up, (c) all `anf-primary-account` ANF volumes (backed up by ANF snapshot replication, not Commvault), or (d) a subclient match was found but the Data Protection report has no recent backup date for it (retired / SLA-excluded subclients) |

### dfs_mapping.csv

| Column | Type | Description |
|--------|------|-------------|
| `dfs_path` | string | DFS namespace path e.g. `\\corp.example.com\BosProd\Current` |
...
```m
let
    Source = SharePoint.Files("https://yourorg.sharepoint.com/sites/teams_platformengineering", [ApiVersion = 15]),
    Filtered = Table.SelectRows(Source, each [Folder Path] = "https://yourorg.sharepoint.com/sites/teams_platformengineering/Shared Documents/Storage-Reporting/volume-data/"),
...
> **Why `vol_key` instead of `volume_name`:** Both `anf-dr-account` and `anf-primary-account` contain volumes with identical names (e.g. `eu2-x-prod`). `volume_name` alone is not unique — `vol_key` combines platform + cluster + volume_name to guarantee uniqueness.
...
-- (anf-dr-account/quant_standard and anf-primary-account/quant_standard are distinct pools)
VAR LatestDate = CALCULATE(MAX(volume_snapshots[snapshot_date]), ALL(volume_snapshots))
RETURN
CALCULATE(
    SUMX(
...
| Data source authentication | OAuth2 (Corporate Microsoft account), Privacy level: Organizational |
...
**Root cause:** The Azure ARM API returns pool names in the format `accountName/poolName` (e.g. `anf-dr-account/PremiumPool`). The code was passing the full compound name to the volumes API, resulting in a URL like `.../capacityPools/anf-dr-account%2FPremiumPool/volumes` which is invalid.
...
pool_name = pool["name"]  # "anf-dr-account/PremiumPool"
...
**Root cause:** The SSH private key for `svc-netapp` lives at `/opt/svc-netapp/.ssh/id_rsa` on Linux servers. The Windows build agent has no access to that NFS mount.

**Fix:**
1. Uploaded the private key file to **Azure DevOps Secure Files** as `svc-netapp_id_rsa`.
...
**Symptom:** `_token_mgr.token_data` was empty. The log showed the token manager could not read the token file at `\fileserver\automation\backupautomation\commvault\.commvault_tokens.json`.

**Root cause:** The `svc-prdtfsbld` Windows service account running the build agent does not have access to the NFS-backed UNC path `\fileserver\automation\...` where the Commvault tokens are stored. That path is designed for the Linux automation server (`linux-automation-server`).
...
```
[ERROR] Failed to save tokens: [WinError 183] Cannot create a file when that file already exists: '\fileserver\automation\backupautomation\commvault'
```
...
- **Issue 13:** ANF Commvault clients (`anf-XXXX.corp.example.com`) were excluded from keyword filter — fixed by adding `"anf"`.
...
**Root cause:** The `commvault-server` Commvault v11.40.26 server has slow subclient detail endpoints. The `v2/Subclient` bulk endpoint requires admin rights not held by the service account. The `v1/Subclient/{id}` endpoint times out under load.
...
1. **SubClient Configuration Information (Content, Filter and Filter Exception)**
   - Saved to: `\fileserver\automation\backupautomation\commvault\reports`
   - Columns: Client, Workload, Instance, Subclient, Content
   - Content field contains comma-separated UNC paths such as `UNC-NT_anf-source.corp.example.com\eu2-x-ats-betas`
...
**Symptom:** All 26 volumes in `anf-dr-account` showed `backup_configured=False` even though the Commvault SubClient Configuration report clearly listed UNC paths like `UNC-NT_anf-source.corp.example.com\eu2-x-ats-betas`.

**Root cause:** The assumption was that CUS (Central US) ANF volumes would be named with a `cus-` prefix. In reality, both `anf-dr-account` and `anf-primary-account` host volumes with `eu2-*` naming. The two accounts contain the same volume names but at different service levels (Standard vs Premium) and potentially in different regions. Commvault only backs up the `anf-dr-account` copies; the `anf-primary-account` copies are marked NA by the business-rule override.
...
|-----------|--------|-------|
| Azure AD app registration | Working | `Corp Storage Volume Reporting`, admin consent granted |
...
| ANF collection | Working | 55 volumes (anf-dr-account: 26, anf-primary-account: 29) — all RW |
...
- Commvault mapper: CSV-report based (no API calls)
  - 58 True / 55 False / 234 NA (all DP/LS → NA, all anf-primary-account → NA)
...
| All `anf-primary-account` ANF volumes (no Commvault backup by design) | 29 |
