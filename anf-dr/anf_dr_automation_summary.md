...
| Subscription ID | `c303bd32-eddf-42ca-9946-d679e0b1e1f3` |
| Tenant ID | `337b9f7b-9e69-4689-9b0d-3417bd3d8566` |

### Production Environment
| Role | Account | Resource Group | Pool |
|---|---|---|---|
| Source (EU2) | `eu2prdanf01` | `eastus2-eu2prdanf01-rg` | `quant_standard` |
| DR (CUS) | `cusprdanf01` | `centralus-cusprdanf01-rg` | `quant_standard` |

### Test Environment (used for DR test)
| Role | Account | Resource Group | Pool | QoS Type |
|---|---|---|---|---|
| Source (EU2) | `eu2tstanf01` | `eastus2-eu2tstanf01-rg` | `test_pool` | Manual |
| DR (CUS) | `custstanf01` | `centralus-custstanf01-rg` | `test` | **Auto** |

### DFS Namespace
| Field | Value |
|---|---|
| Namespace Server | `\\gmo.tld\TS2` (Domain V2, 5 root targets: AUE, ASH, MAR, EU2, CUS) |
| Test Folder | `\\gmo.tld\TS2\DR_Test` |
| EU2 Target | `\\anf-9434.gmo.tld\eu2-dr-test` → IP `10.98.239.9` |
| CUS Target | `\\anf-72cf.gmo.tld\cus-dr-test` → IP `100.64.1.120` |
...
| Refresh command | `Get-Credential -UserName 'gmo\admin-smohanty' \| Export-Clixml -Path "$env:USERPROFILE\.gmo_admin_cred.xml"` |
...
|---|---|---|
| DFS target enable/disable | `gmo\admin-smohanty` (via stored cred + local Start-Process) | `gmo\smohanty` lacks write delegation on DFS namespace |
| ANF BreakReplication / ResyncReplication | Elevated RBAC (Contributor or NetApp Account Contributor on `centralus-custstanf01-rg`) | Standard user lacks `breakReplication/action` on the DR resource group |
...
**Usage:**
```powershell
.\Get-DFSTargets.ps1 -DFSFolderPath "\\gmo.tld\DR\test"
```
...
DFS write operations require `admin-smohanty` credentials. `Invoke-Command -ComputerName gmo.tld -Credential` and `New-CimSession` (both WinRM and DCOM) all return "Access is denied" due to WinRM restrictions in the environment. The GUI (DFS Management MMC) works because it uses native RPC/DCOM from the local process token. Solution: `Start-Process pwsh -Credential $adminCred` spawns a local PowerShell process running as `admin-smohanty` — identical auth path to the GUI. An inner `.ps1` script is written to the Logs folder and executed by the spawned process.

**Credential Auto-Load (March 3):**
Credentials for `admin-smohanty` are stored once via DPAPI (`Export-Clixml`) to `~\.gmo_admin_cred.xml`. The script auto-loads them on Enable/Disable — no interactive prompt needed. Falls back to `Get-Credential` if the file is missing.
...
**Usage (explicit mode):**
```powershell
.\Manage-DFSPath.ps1 -Action Enable -DFSFolderPath "\\gmo.tld\TS2\DR_Test" -TargetPaths "\\anf-72cf.gmo.tld\cus-dr-test"
```
...
|---|---|---|---|
| `Production` (default) | `eu2prdanf01` | `cusprdanf01` | `quant_standard` |
...
.\Create-ANFVolumes.ps1 `
    -SourceResourceGroup "eastus2-eu2tstanf01-rg" -SourceAccountName "eu2tstanf01" -SourcePoolName "test_pool" `
    -DRResourceGroup "centralus-custstanf01-rg" -DRAccountName "custstanf01" -DRPoolName "test"
```
...
| 11 | `gmo\smohanty` cannot change DFS target state | Account lacks write delegation on `\\gmo.tld\DR` namespace | Must use `service-Backup` / `admin-smohanty` session for all DFS changes |
| 12 | `gmo\smohanty` cannot break ANF replication | Lacks `breakReplication/action` RBAC on `centralus-custstanf01-rg` | Elevated role (Contributor/NetApp Account Contributor) via PIM required |
| 13 | DNS not resolving `anf-9434.gmo.tld` from workstation | Workstation DNS set to public resolvers (`75.75.75.75`) not Infoblox | Registered A record in Infoblox; corporate machines resolve correctly. Workstation resolved via Comcast DNS cache after flush |
...
| 16 | `Invoke-Command -ComputerName gmo.tld -Credential admin-smohanty` → "Access is denied" | WinRM is restricted in the environment; outbound WinRM to domain controllers/DFS servers is blocked for standard accounts | Replaced with `Start-Process pwsh -Credential` — spawns local process as admin-smohanty using native RPC (identical to DFS Management GUI) |
| 17 | `New-CimSession -Protocol Dcom -Credential admin-smohanty` → "Access is denied" | DCOM also blocked for explicit credentials across the network in this environment | Same fix as #16 — local Start-Process |
...
| 20 | `Set-DfsnFolderTarget` as `smohanty` → "Access to a CIM resource was not available" | `smohanty` lacks DFS namespace write delegation | Must run DFS writes as `admin-smohanty` — stored credential + Start-Process approach |
...
### Test Setup
- Source volume: `eu2-dr-test` on `eu2tstanf01/test_pool` (EU2)
- DR volume: `cus-dr-test` on `custstanf01/test` (CUS) — DataProtection type
- DFS folder: `\\gmo.tld\DR\test` with both targets registered
- DNS: `anf-9434.gmo.tld` → `10.98.239.9` registered in Infoblox
...
Action: Set `\\anf-9434.gmo.tld\eu2-dr-test` to Offline
Run from: `service-Backup` session (gmo\smohanty lacks DFS delegation)
```powershell
Set-DfsnFolderTarget -Path "\\gmo.tld\DR\test" -TargetPath "\\anf-9434.gmo.tld\eu2-dr-test" -State Offline
```
...
Action: Set `\\anf-72cf.gmo.tld\cus-dr-test` to Online
Run from: `service-Backup` session
```powershell
Set-DfsnFolderTarget -Path "\\gmo.tld\DR\test" -TargetPath "\\anf-72cf.gmo.tld\cus-dr-test" -State Online
```
...
```powershell
Get-ChildItem "\\anf-72cf.gmo.tld\cus-dr-test" | Format-Table Name, Length, LastWriteTime -AutoSize
Get-Content "\\anf-72cf.gmo.tld\cus-dr-test\DR-Test-File.txt"
```
```
...
```powershell
"This file was written to the DR volume (CUS) on 2026-02-20 13:08:22 AFTER replication break. This will be DELETED after resync." | Set-Content "\\anf-72cf.gmo.tld\cus-dr-test\DR-Only-File.txt"
```
```
Name             Length LastWriteTime
...
Run from: `service-Backup` session
```powershell
Set-DfsnFolderTarget -Path "\\gmo.tld\DR\test" -TargetPath "\\anf-72cf.gmo.tld\cus-dr-test" -State Offline
```
...
Run from: `service-Backup` session
```powershell
Set-DfsnFolderTarget -Path "\\gmo.tld\DR\test" -TargetPath "\\anf-9434.gmo.tld\eu2-dr-test" -State Online
```
```
Path              TargetPath                     State  ReferralPriorityClass
\\gmo.tld\DR\test \\anf-9434.gmo.tld\eu2-dr-test Online sitecost-normal
```
...
### Prerequisites
- Stored credential at `~\.gmo_admin_cred.xml` (run once: `Get-Credential -UserName 'gmo\admin-smohanty' | Export-Clixml -Path "$env:USERPROFILE\.gmo_admin_cred.xml"`)
- Elevated RBAC on `centralus-cusprdanf01-rg` for ANF break/resync (via PIM if needed)
...
Test,\\gmo.tld\TS2\DR_Test,\\anf-9434.gmo.tld\eu2-dr-test,\\anf-72cf.gmo.tld\cus-dr-test,cus-dr-test
Production,\\gmo.tld\<namespace>\<folder>,\\<eu2-anf>\<share>,\\<cus-anf>\<share>,<volume-name>
```
...
> **DFS Permission Requirement:** DFS write operations (Enable/Disable) require `gmo\admin-smohanty`. The script auto-loads the stored DPAPI credential from `~\.gmo_admin_cred.xml` — no interactive prompt. Internally, it spawns a local `pwsh` process as `admin-smohanty` using `Start-Process -Credential` (native RPC, same as DFS Management GUI). `Invoke-Command` and `CimSession` are not used — both are blocked by WinRM restrictions in this environment.
...
> **Stored Credential Refresh:** If `admin-smohanty` password changes, refresh the stored credential: `Get-Credential -UserName 'gmo\admin-smohanty' | Export-Clixml -Path "$env:USERPROFILE\.gmo_admin_cred.xml"`. The file is DPAPI-encrypted — only `smohanty` on this machine can read it.

> **DNS Note:** ANF volume FQDNs (e.g. `anf-9434.gmo.tld`) are registered in Infoblox. Workstations using public DNS resolvers (Comcast: `75.75.75.75`) may not resolve them. Corporate machines using Infoblox (`10.200.249.10`) resolve correctly. DFS path access via `\\gmo.tld\TS2\...` works regardless as the DFS server handles resolution.
