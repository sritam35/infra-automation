...
| Field | Value |
|---|---|
| Subscription Name | corp-primary |
...
| Refresh command | `Get-Credential -UserName 'corp\admin-username' \| Export-Clixml -Path "$env:USERPROFILE\.admin_cred.xml"` |
...
|---|---|---|
| DFS target enable/disable | `corp\admin-username` (via stored cred + local Start-Process) | `corp\current-user` lacks write delegation on DFS namespace |
...
| DFS GetStatus | `corp\current-user` (current session) | Read-only; native RPC works without elevated rights |
...
Credentials for `admin-username` are stored once via DPAPI (`Export-Clixml`) to `~\.admin_cred.xml`. The script auto-loads them on Enable/Disable — no interactive prompt needed. Falls back to `Get-Credential` if the file is missing.
...
| 11 | `corp\current-user` cannot change DFS target state | Account lacks write delegation on `\\corp.example.com\DR` namespace | Must use `service-Backup` / `admin-username` session for all DFS changes |
| 12 | `corp\current-user` cannot break ANF replication | Lacks `breakReplication/action` RBAC on `centralus-anf-dr-test-account-rg` | Elevated role (Contributor/NetApp Account Contributor) via PIM required |
...
Run from: `service-Backup` session (corp\current-user lacks DFS delegation)
```powershell
...
```
**First attempt failed** — `corp\current-user` did not have `breakReplication/action` RBAC. Re-ran after elevating role via PIM (~2 minute propagation + reconnect).
```
...
| Mar 3 | Manage-DFSPath.ps1 | Added DPAPI stored credential auto-load from `~\.admin_cred.xml`; falls back to `Get-Credential` prompt if file absent |
...
### Prerequisites
- Stored credential at `~\.admin_cred.xml` (run once: `Get-Credential -UserName 'corp\admin-username' | Export-Clixml -Path "$env:USERPROFILE\.admin_cred.xml"`)
...
> **Stored Credential Refresh:** If `admin-username` password changes, refresh the stored credential: `Get-Credential -UserName 'corp\admin-username' | Export-Clixml -Path "$env:USERPROFILE\.admin_cred.xml"`. The file is DPAPI-encrypted — only `smohanty` on this machine can read it.
