...
- Stored credential file `~\.gmo_admin_cred.xml` present (see setup note below)
...
- DFS admin rights (`gmo\admin-<username>`)

**One-time credential setup (per user, per machine):**

```powershell
# Save DFS admin credential (only needs to be done once per machine)
Get-Credential -Message "Enter gmo\admin-<username>" |
    Export-Clixml -Path "$HOME\.gmo_admin_cred.xml"
...
```
┌─────────────────────────────────────────────────────────────────┐
│                    DFS Namespace: \\gmo.tld\TS2                 │
│                                                                 │
│   \\gmo.tld\TS2\<FolderName>                                    │
│        │                                                        │
│        ├── \\anf-XXXX.gmo.tld\<volume>  (EU2 Source)  ◄──────┐ │
│        └── \\anf-XXXX.gmo.tld\<volume>  (CUS DR)             │ │
└──────────────────────────────────────────────────────────────│─┘
                                                               │
┌─────────────────────────┐      CRR Replication      ┌───────┴───────────────┐
...
```
  ...
  ──────────────────────────────────────────────────────────────────────────
  [v]   EU2 Source   \\anf-XXXX.gmo.tld\<volume>               Online     YES  <--
  [ ]   CUS DR       \\anf-XXXX.gmo.tld\<volume>               Offline    no
...
```
  DFS Target Status
  ──────────────────────────────────────────────────────────────────────────
  [v]   EU2 Source   \\anf-XXXX.gmo.tld\<volume>               Offline
  [v]   CUS DR       \\anf-XXXX.gmo.tld\<volume>               Online
...
```
  DFS Target Status
  ──────────────────────────────────────────────────────────────────────────
  [v]   EU2 Source   \\anf-XXXX.gmo.tld\<volume>               Online
  [v]   CUS DR       \\anf-XXXX.gmo.tld\<volume>               Offline
...
- **Credential binding:** The stored credential (`~\.gmo_admin_cred.xml`) is encrypted with Windows DPAPI and is tied to the user and machine it was created on. It must be re-created on any new machine or for any new user who needs to run the scripts.
