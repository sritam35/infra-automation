...
.DESCRIPTION
    ...
    Enable/Disable run as gmo\admin-smohanty via Invoke-Command to the
...
.PARAMETER TargetPaths
    ...
    Example: @("\\anf-9434.gmo.tld\eu2-dr-test", "\\anf-72cf.gmo.tld\cus-dr-test")

.PARAMETER Credential
    Credentials to use for Enable/Disable operations.
    Defaults to prompting for gmo\admin-smohanty.
...
.EXAMPLE
    ...
    .\Manage-DFSPath.ps1 -Action GetStatus -DFSFolderPath "\\gmo.tld\TS2\DR_Test"

.EXAMPLE
    # Disable source target before DR failover
    .\Manage-DFSPath.ps1 -Action Disable -DFSFolderPath "\\gmo.tld\DR\test" -TargetPaths "\\anf-9434.gmo.tld\eu2-dr-test"

.EXAMPLE
    # Enable DR target after breaking replication
    .\Manage-DFSPath.ps1 -Action Enable -DFSFolderPath "\\gmo.tld\DR\test" -TargetPaths "\\anf-72cf.gmo.tld\cus-dr-test"

.EXAMPLE
    # Disable multiple targets at once
    .\Manage-DFSPath.ps1 -Action Disable -DFSFolderPath "\\gmo.tld\prd_eu2\Opr" -TargetPaths @("\\anf-72b8.gmo.tld\eu2-b-prdopr", "\\anf-8dcf.gmo.tld\cus-b-prdopr")

.EXAMPLE
    # Preview changes without applying
    .\Manage-DFSPath.ps1 -Action Disable -DFSFolderPath "\\gmo.tld\DR\test" -TargetPaths "\\anf-9434.gmo.tld\eu2-dr-test" -WhatIf
...
function Show-Help {
    ...
    Write-Information "  Enable/Disable : Runs as gmo\admin-smohanty via Invoke-Command."
    Write-Information ""
    ...
    Write-Information "  -Credential      PSCredential for Enable/Disable  (prompts for gmo\admin-smohanty if omitted)"
    ...
    Write-Information "  .\Manage-DFSPath.ps1 -Action GetStatus -DFSFolderPath `"\\gmo.tld\TS2\DR_Test`""
    Write-Information ""
    Write-Information "  # Disable EU2 source target (step 1 of failover)"
    Write-Information "  .\Manage-DFSPath.ps1 -Action Disable -DFSFolderPath `"\\gmo.tld\TS2\DR_Test`" -TargetPaths `"\\anf-9434.gmo.tld\eu2-dr-test`""
    Write-Information ""
    Write-Information "  # Enable CUS DR target (step 2 of failover)"
    Write-Information "  .\Manage-DFSPath.ps1 -Action Enable -DFSFolderPath `"\\gmo.tld\TS2\DR_Test`" -TargetPaths `"\\anf-72cf.gmo.tld\cus-dr-test`""
    Write-Information ""
    Write-Information "  # Disable multiple targets"
    Write-Information "  .\Manage-DFSPath.ps1 -Action Disable -DFSFolderPath `"\\gmo.tld\prd_eu2\Opr`" -TargetPaths @(`"\\anf-72b8.gmo.tld\eu2-b-prdopr`", `"\\anf-8dcf.gmo.tld\cus-b-prdopr`")"
    Write-Information ""
    Write-Information "  # Preview without applying"
    Write-Information "  .\Manage-DFSPath.ps1 -Action Disable -DFSFolderPath `"\\gmo.tld\DR\test`" -TargetPaths `"\\anf-9434.gmo.tld\eu2-dr-test`" -WhatIf"
    Write-Information ""
...
if ($Action -ne "GetStatus" -and -not $Credential) {
    if (Test-Path $credFile) {
        ...
        Write-Information "[TIP]  Store once: Get-Credential -UserName 'gmo\admin-smohanty' | Export-Clixml -Path '$credFile'" -InformationAction Continue
        $Credential = Get-Credential -UserName "gmo\admin-smohanty" -Message "Enter admin-smohanty credentials for DFS $Action operation"
    }
}
...
foreach ($folderPath in $uniqueFolders) {
    ...
    # ==========================================
    # STEP 3: Apply state changes as admin-smohanty (local Start-Process, no WinRM)
    # ==========================================
