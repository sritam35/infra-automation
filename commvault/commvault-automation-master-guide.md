...
| Role | Server |
|------|--------|
| Commvault Server | bedprdbck001 |
| Linux Automation Server | marprdnfs001 |

**Deployment Path:** `/mnt/global/nfs/backupautomation/commvault`
**Network (UNC) Path:** `\\gmo\dsl\backupautomation\commvault`
**Scripts Location (Tidal Jobs):** `/mnt/global/nfs/backupautomation/commvault2/`
...
Trigger Release Pipeline (Backup Automation Deployment)
    ↓
Deploy to Linux via SSH (as svc_netapp)
    ↓
Scripts Ready on marprdnfs001 with Correct Ownership
    ↓
...
| Deployment Method | **Copy files over SSH** (as `svc_netapp`) |
| Deployment Target | `/mnt/global/nfs/backupautomation/commvault` |
| SSH Connection | `gmo-linux-prd` (user: svc_netapp) |

> **Important:** Earlier pipeline versions used Windows UNC path copy (`\\gmo\dsl\backupautomation\commvault`), which caused NFS ownership issues. The pipeline now uses SSH-based deployment. See [Issue #10](#issue-10--file-ownership-and-permission-issues-from-windows-deployment) for full details.
...
Ensure the NetApp volume is mounted on marprdnfs001 before deploying:
...
# Expected output:
# marprdsmb32_ha1a_nfs:/marprdsmb32_t0_backupautomation_vol/commvault  9.5G  6.0M  9.5G   1% /mnt/global/nfs/backupautomation/commvault
```
...
| Mount Point | `/mnt/global/nfs/backupautomation/commvault` |
| Size | 9.5 GB |

### Azure DevOps Service Connections Required

- **SharedBuild agent**: Read access to repository
- **DevOpsPRD (DOPSPRD) agent**: Can establish SSH connection using the `gmo-linux-prd` service connection
- **`gmo-linux-prd` SSH connection**: Authenticates to marprdnfs001 as `svc_netapp`
...
```bash
# SSH to marprdnfs001
ssh root@marprdnfs001

# Navigate to deployment location
cd /mnt/global/nfs/backupautomation/commvault

# List deployed files (expected ownership: svc_netapp)
ls -la

# Expected output:
# -rwx------ 1 svc_netapp gmo  5196 Feb 16 09:54 check_token_health.py
# -rwx------ 1 svc_netapp gmo  3326 Feb 16 09:54 commvault_get_api.py
# -rwx------ 1 svc_netapp gmo 56084 Feb 16 09:54 commvault_post_api.py
# -rwx------ 1 svc_netapp gmo  ...  Feb 16 09:54 dynamic_datalakebkp_exclusions.py
# -rwx------ 1 svc_netapp gmo  ...  Feb 16 09:54 renew_token.py
# -rwx------ 1 svc_netapp gmo  ...  Feb 16 09:54 token_manager.py
```
...
1. **Only ONE machine should manage tokens** — whichever machine renews first invalidates the token on all others. The production Linux server (`marprdnfs001`) is the single source of truth.
2. Token file (`.commvault_tokens.json`) must be owned by `svc_netapp` (the Tidal service account).
...
```bash
su - svc_netapp
cd /mnt/global/nfs/backupautomation/commvault2
...
```bash
python3 token_manager.py "<access_token>" "<refresh_token>"
chown svc_netapp:svc_netapp .commvault_tokens.json
```

### Commvault API Endpoint Notes

The working API endpoint for token renewal on `bedprdbck001` is:

```
POST https://bedprdbck001/commandcenter/api/v4/AccessToken/Renew
```
...
|----------|---------|------------|----------|
| `110_commvault_token_renewal` | `/opt/tidalprd_linux/remote_shell.sh` | `-u 'svc_netapp' -h marprdnfs001 -s 'python3 /mnt/global/nfs/backupautomation/commvault2/renew_token.py'` | Every Sunday 00:00 |
| `111_datalake_backup_exclusion` | `/opt/tidalprd_linux/remote_shell.sh` | `-u 'svc_netapp' -h marprdnfs001 -s 'python3 /mnt/global/nfs/backupautomation/commvault2/dynamic_datalakebkp_exclusions.py'` | 1st of every month 00:30 |
...
```
POST https://bedprdbck001/webservice/V4/AccessToken/Renew
```
...
```
/mnt/global/nfs/storageautomation/automation/commvault/.commvault_tokens.json
```
But scripts were deployed to:
```
/mnt/global/nfs/backupautomation/commvault2/
```

**Fix:**
```python
# Before:
TOKEN_FILE = "/mnt/global/nfs/storageautomation/automation/commvault/.commvault_tokens.json"

# After:
TOKEN_FILE = "/mnt/global/nfs/backupautomation/commvault2/.commvault_tokens.json"
```

---

### Issue #8 — Token File Permission Denied (Tidal/svc_netapp)
**Date:** February 13, 2026 | **Severity:** High
**Script Affected:** `token_manager.py` (runtime)

**Problem:**
Token file was initialized as `root` with `chmod 600`. When Tidal ran the script as `svc_netapp`, it received:
```
[WARNING] Failed to load tokens: [Errno 13] Permission denied:
'/mnt/global/nfs/backupautomation/commvault2/.commvault_tokens.json'
```

**Fix:** Re-initialize tokens as `svc_netapp`:
```bash
# Option A: Initialize directly as svc_netapp
su - svc_netapp
cd /mnt/global/nfs/backupautomation/commvault2
python3 token_manager.py "<access_token>" "<refresh_token>"
...
**Rule:** Only ONE machine manages token lifecycle. `marprdnfs001` is the single source of truth.
...
**Problem:**
The release pipeline used Windows file copy (UNC path `\\gmo\dsl\backupautomation\commvault`). Files landed on the Linux NFS mount with numeric UID ownership instead of `svc_netapp`:

```bash
ls -l /mnt/global/nfs/backupautomation/commvault
-rwx------ 1 1631689629 gmo 5196 Feb 13 11:51 check_token_health.py
```
- Owner: numeric UID `1631689629` (Windows SID → Linux UID mapping)
- Should be: `svc_netapp`
...
| Target path | `\\gmo\dsl\backupautomation\commvault` | `/mnt/global/nfs/backupautomation/commvault` |
| SSH connection | N/A | `gmo-linux-prd` (as `svc_netapp`) |
...
-rwx------ 1 svc_netapp gmo 56084 Feb 16 09:54 commvault_post_api.py
```
✅ Owner: `svc_netapp` | ✅ Group: `gmo` | ✅ Permissions: `700`
✅ Tidal scheduler executes scripts directly — no manual workaround needed

**Why SSH works:** Files copied via SSH as `svc_netapp` are created with `svc_netapp` ownership from the start. NFS security then applies to the correct owner, and no post-deployment changes are needed.
...
| Issue | Solution |
|-------|----------|
| SSH copy task fails with "Access denied" | Verify `gmo-linux-prd` SSH service connection is active and the target directory exists on marprdnfs001 |
...
| Permission denied on token file | Token file not owned by `svc_netapp`; re-initialize under that account or `chown` the file |
...
| Release Pipeline (SSH) | `gmo-linux-prd` connection (as `svc_netapp`) |
| Token file | `chmod 600`, owned by `svc_netapp` |
...
6. **Service account permissions matter** — Token files must be owned by the user Tidal runs scripts as (`svc_netapp`).
...
# Switch to svc_netapp for token init
su - svc_netapp
cd /mnt/global/nfs/backupautomation/commvault2
...
|----------|------|-----------------|
| Storage Automation | `Pipelines/storage-automation-pipeline.yml` | `Automation/*` → `/mnt/global/nfs/storageautomation/automation2` |
| Storage Monitoring | `Pipelines/storage-monitoring-pipeline.yml` | `Monitoring/*` → `/mnt/global/nfs/storageautomation/outputs` |
...
| NFS mount issues on marprdnfs001 | Storage Team |
